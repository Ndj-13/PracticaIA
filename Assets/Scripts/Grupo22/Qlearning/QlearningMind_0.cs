...
{
    public class QlearningMind_0 : AbstractPathMind
    {
        ...
        public override Locomotion.MoveDirection GetNextMove(BoardInfo boardInfo, CellInfo currentPos, CellInfo[] goals)
        {
            ...
            {
                ficherotablaQ = new FileStream("./Assets/Scripts/Grupo22/Qlearning/tableQ.txt", FileMode.Open, FileAccess.Read);
            }

            float [,] tablaQ = arrayTablaQ(ficherotablaQ);
            //for (int i = 0; i < 8 * 14; i++)
            //{
            //    for (int j = 0; j < 4; j++)
            //    {
            //        Debug.Log(tablaQ[i, j]);
            //    }
            //}
            //2º Exploracion 100%
        ...
        private float[,] arrayTablaQ(FileStream fichero)
        {
            ...
            try
            {
                using (StreamReader buscar = new StreamReader(fichero))
                {
                    int linea = 0;
        ...
        private void exploracionQlearning(BoardInfo board, float[,] tablaQ)
        {
            //Episodios: 
            int N_EPI_MAX = 10;
            //Celdas que recorrer:
            ...
            for (int k = 0; k < N_EPI_MAX; k++)
            {
                ...
                while (!stop_condition)
                {
                  ...
                  //float next_Qmax = Get_maxQ(tablaQ, next_cell); //miramos cual es el valor +alto q hay de Q en la siguiente habitacion
        ...
        private float Get_maxQ(FileStream tableQ, CellInfo next_cell)
        {
            //Variable para buscar valor
            string line = "Not found";
            float maxQ = -1;
            ...
            try
            {
                using (StreamReader buscar = new StreamReader(tablaQ))
                {
                    ...
                    while (!buscar.EndOfStream && fila <= next_cell.RowId * 14)
                    {
                        int i = 0;
                        while (i < 14)
                        {
                    ...
                    //Ya hemos llegado a la fila, falta la columna
                    while (!buscar.EndOfStream && columna <= next_cell.ColumnId)
                    {
                        line = buscar.ReadLine();
                        columna++;
                    }
                    ...
                    //[3] = 2 (S)
                    //[4] = 3 (O)
                    int accion = 1; //nos saltamos la primera posicion que son las letras

                    while (accion <= celda.Length)
                    {
                        if (float.Parse(celda[accion]) > maxQ) maxQ = float.Parse(celda[accion]);
                        accion++;
                    }
